...
async function fetchList() {
  loading.value = true
  try {
    const data = await useAPI('/api/admin/security/allowlist')
...
async function addItem() {
  ...
  adding.value = true
  try {
    const result = await useAPI('/api/admin/security/allowlist', {
      method: 'POST',
      body: {
  ...
  <Card>
    <CardHeader class="flex flex-row items-center justify-between">
      <div>
        ...
        <CardDescription>白名单 IP 完全免疫所有限流和黑名单</CardDescription>
      </div>
          ...
          <DialogContent class="max-w-md">
            ...
            <div class="space-y-4 py-2">
              ...
              <div class="space-y-2">
                ...
                <Input v-model="newNote" placeholder="例如: 办公室出口 IP" />
              </div>
            </div>
            <DialogFooter>
    ...
    <CardContent>
      ...
      <div v-else class="grid gap-2 sm:grid-cols-2 max-h-96 overflow-y-auto">
        <div
          v-for="item in items"
        ...
        >
          <div class="flex-1 min-w-0">
            ...
            <div class="text-xs text-muted-foreground mt-1 truncate">
              由 {{ item.addedBy }} 于 {{ formatTime(item.addedAt) }}
