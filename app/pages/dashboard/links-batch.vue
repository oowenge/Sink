...
async function submit() {
  ...
  try {
    ...
    toast({
      title: '批量生成完成',
      description: `${data.successCount} 成功 / ${data.failureCount} 失败`,
    })
  }
  catch (err: any) {
    toast({
...
<template>
  <div class="container max-w-5xl py-6 space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">批量生成短链</h1>
      <NuxtLink to="/dashboard/links" class="text-sm text-muted-foreground hover:text-primary">
        ← 返回链接列表
      </NuxtLink>
    </div>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          每行一条。支持三种格式:<code>url</code> / <code>url,slug</code> / <code>url,slug,comment</code>。
          slug 留空将自动生成。最多一次 500 条。
        </CardDescription>
      </CardHeader>
...
https://example.com/page-3,promo,Spring sale"
